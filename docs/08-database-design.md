...
Thiết kế logic, chưa phải migration. MySQL/InnoDB; phiên bản mục tiêu cần chốt Q-016, phải hỗ trợ CHECK được thực thi. PK mặc định `id BIGINT UNSIGNED`; API biểu diễn ID thành chuỗi để tránh mất chính xác JavaScript. Các bảng có `created_at DATETIME(6)` UTC, bảng có thay đổi thêm `updated_at`. FK được index; FK lịch sử dùng RESTRICT, không cascade xóa giao dịch.
...
| SeatHoldItem — giá đã báo | hold_id FK SeatHold, session_seat_id FK SessionSeat, price_minor, currency | UQ `(hold_id,session_seat_id)`; price >= 0 | `(session_seat_id,id)` giữ lịch sử |
| Booking — đơn | customer_id FK User, session_id FK EventSession, hold_id FK SeatHold, status, total_minor, currency, expires_at, confirmed_at nullable | UQ hold_id; tổng tính từ item (service transaction) | `(customer_id,created_at,id)`, `(status,expires_at,id)`, `(session_id,status,id)` |
| BookingItem — dòng đơn | booking_id FK Booking, session_seat_id FK SessionSeat, price_minor, currency, seat_label_snapshot | UQ `(booking_id,session_seat_id)` | `(session_seat_id,id)` tra lịch sử |
| Ticket — quyền vào cửa | booking_item_id FK BookingItem, qr_token_hash, qr_token_ciphertext, status, checked_in_at nullable, checked_in_by FK User nullable | UQ booking_item_id; UQ qr_token_hash; USED cần thời gian/người quét | `(status,id)` job expiry; `(checked_in_by,checked_in_at)` audit |
...
| PaymentWebhook — hộp nhận callback | provider, provider_event_id, payment_id FK Payment nullable, payload_hash, status, received_at, processed_at nullable | UQ `(provider,provider_event_id)`; không lưu payload bí mật nguyên bản | `(status,received_at,id)` retry |
| OutboxMessage — công việc sau commit | type, aggregate_type, aggregate_id, aggregate_version, dedupe_key, payload_minimal, status, attempts, available_at, lease_until nullable, last_error_code nullable | UQ dedupe_key; status PENDING/PROCESSING/DONE/FAILED | `(status,available_at,id)`, `(status,lease_until,id)` thu hồi lease |
...
- Session-seat phải thuộc venue của session; hold/booking/item phải cùng session và cùng chủ. Đây là ràng buộc liên bảng không thể biểu diễn bằng CHECK hàng đơn giản; service kiểm tra dưới khóa và integration test bắt buộc. Composite FK có thể bổ sung khi triển khai nếu denormalize khóa có lý do.
...
- Payment dùng cột generated nullable `pending_booking_guard`: booking_id khi PENDING, NULL ở trạng thái khác; UQ bảo vệ tối đa một attempt chờ. Tương tự `successful_booking_guard` cho SUCCESS/REFUNDED (và PARTIALLY_REFUNDED khi được thêm). Kết quả tiền xung đột guard phải được giữ trong inbox và đưa vào đối soát, không bỏ mất bằng lỗi unique.
...
- Quyền vé khi hoàn/hủy và SessionSeat phải đổi cùng transaction. Chỉ status VALID/USED được coi là đã cấp quyền; bất biến nhiều vé hiện hành trên một ghế kiểm thử bằng join Ticket → BookingItem → SessionSeat.
