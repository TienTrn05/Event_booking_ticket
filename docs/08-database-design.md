...
Thiết kế logic đã cập nhật Organization/review/layout/Google-OTP/check-in theo [23](23-organization-review-seatmap.md). [database/schema.sql](../database/schema.sql) và ERD xuất sẵn vẫn là baseline trước cập nhật này, chưa dùng làm migration hiện hành; checklist đồng bộ tại [20](20-physical-sql-design.md). MySQL/InnoDB; phiên bản production cần chốt Q-016, phải hỗ trợ CHECK được thực thi; DDL đã kiểm tra trên MySQL 8.0.46. PK mặc định `id BIGINT UNSIGNED`; API biểu diễn ID thành chuỗi để tránh mất chính xác JavaScript. Các bảng dùng timestamp UTC; PaymentWebhook dùng received_at thay created_at. FK được index; FK lịch sử dùng RESTRICT, không cascade xóa giao dịch.
...
| Ticket — quyền vào cửa | booking_item_id, session_seat_id qua composite FK, ticket_code_hash/ciphertext, qr_token_hash, qr_token_ciphertext, qr_key_version, status, admitted_at/by nullable | UQ booking_item_id, ticket_code_hash, qr_token_hash; UQ active_seat_guard cho VALID/USED; USED cần thời gian/người quét | `(status,id)` job expiry; `(admitted_by,admitted_at)` audit |
...
| EventReview | event_id,event_version,submitted_by/at,expires_at,status,reviewed_by/at | UQ(event_id,event_version); nullable pending guard UQ(event_id) cho PENDING; status transition/time ở service |
...
Ticket mã nhập tay và QR có thể dùng cùng secret ngẫu nhiên dưới hai cách biểu diễn để tránh hai nguồn quyền; hash lookup và ciphertext phục vụ chủ vé xem lại, không log. CheckIn tồn tại không thay kiểm tra VALID khi vào cửa. Baseline tickets.checked_in_at/by phải đổi thành admitted_at/by cho thao tác USED để tránh nhập nhằng với CheckIn.
