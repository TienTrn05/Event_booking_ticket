...
SELECT ss.id AS session_seat_id, s.number, vr.code AS row_code,
       vs.code AS section_code, s.map_x, s.map_y, s.width, s.height,
       tt.code AS ticket_type_code, tt.name AS ticket_type_name,
...
JOIN layout_rows AS vr ON vr.id = s.row_id
JOIN layout_sections AS vs ON vs.id = vr.section_id
...
JOIN seat_layouts AS sl ON sl.id = es.layout_id
JOIN ticket_types AS tt ON tt.id = ss.ticket_type_id
WHERE ss.session_id = @session_id
  AND e.status = 'PUBLISHED' AND es.status = 'SCHEDULED'
  AND o.status = 'APPROVED' AND sl.status = 'FROZEN' AND s.is_active = 1
